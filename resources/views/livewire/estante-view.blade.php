                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($estantes as $estante)
                            <tr>
                                <td class="px-6 py-4 break-words">{{ $estante->no_estante }}</td>
                                <td class="px-6 py-4 break-words">{{ $estante->descripcion }}</td>
                                <td class="px-6 py-4 break-words">{{ $estante->laboratorio->nombre }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="openVerModal({{ $estante }})"
                                        class=" p-2 bg-red-500 text-white rounded-md ">
                                        Ver
                                    </button> | <button wire:click="openModal({{ $estante }})"
                                        class=" p-2 bg-red-500 text-white rounded-md ">
                                        Editar
                                    </button>
                                </td>
                            ...
                            <div class="m-8 my-20 max-w-[400px] mx-auto">
                                <div class="mb-8">
                                    <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                        {{ isset($miEstante->id) ? 'Actualizar' : 'Crear' }} Estante
                                    </h3>
                                    ...
                                    <form wire:submit.prevent="" class="space-y-4">
                                        <div>
                                            <label for="no_estante"
                                                class="block text-sm font-medium text-gray-700">Número de
                                                Estante</label>
                                            <input disabled type="text" id="no_estante" wire:model.defer="no_estante"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            @error('no_estante')
                                                <span class="text-red-500 text-xs">{{ $message }}</span>
                                            @enderror
                                        ...
                                        <div>
                                            <label for="descripcion"
                                                class="block text-sm font-medium text-gray-700">Descripción</label>
                                            <input disabled type="text" id="descripcion" wire:model.defer="descripcion"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            @error('descripcion')
                                                <span class="text-red-500 text-xs">{{ $message }}</span>
                                            @enderror
                                        ...
                                        <div>
                                            <label for="filas"
                                                class="block text-sm font-medium text-gray-700">Filas</label>
                                            <input disabled type="number" id="filas" wire:model.defer="filas"
                                                min="1" max="7"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            @error('filas')
                                                <span class="text-red-500 text-xs">{{ $message }}</span>
                                        ...
                                        <div>
                                            <label for="columnas"
                                                class="block text-sm font-medium text-gray-700">Columnas</label>
                                            <input disabled type="number" id="columnas" wire:model.defer="columnas"
                                                min="1" max="5"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            @error('columnas')
                                                <span class="text-red-500 text-xs">{{ $message }}</span>
                                        ...
                                        <div>
                                            <label for="laboratorio_id"
                                                class="block text-sm font-medium text-gray-700">Laboratorio</label>
                                            <select disabled id="laboratorio_id" wire:model.defer="laboratorio_id"
                                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                                <option value="">Seleccione un laboratorio</option>
                                                @foreach ($laboratorios as $laboratorio)
                                                    <option value="{{ $laboratorio->id }}">{{ $laboratorio->nombre }}
